...
CachyOS uses graphical Emacs 31 as a normal application process. KDE launches the distribution desktop entry, whose command is `emacs %F`. There is no Emacs daemon, user service, custom desktop entry, or client wrapper. Closing the final frame ends Emacs and its child processes without process-confirmation prompts. Killing a terminal buffer likewise stops its process without prompting. The atomic private snapshot under `$XDG_STATE_HOME/myconfig-emacs/` records workspaces, split topology, native buffer assignments, directories, jobs, restart policies, and process recipes. Reopening restores that structure and starts fresh eligible terminal or AIPanel processes in their recorded buffers; terminal screen memory and process-internal state are not restored. Version 1 snapshots are migrated by retaining the selected tab's layout and moving jobs from every old tab into their workspace.
...
The workbench hierarchy is workspace, then split. Each workspace owns one split layout, its native buffers, process restart metadata, and one AIPanel identity. Every ordinary buffer receives workspace metadata when first used and retains that owner while hidden; only internal and explicitly global buffers remain unowned. Terminals and AIPanel sessions appear as normal workspace buffers in the interface even though their restart recipes are tracked internally. Durable workspace-owned file, Dired, and scratch buffers are restored after a restart, while transient owned buffers exist only for the current process. Hidden owned buffers remain assigned to their workspace, and `Space b` excludes buffers owned by other workspaces. The full-frame navigator uses one line per visible buffer, adding split numbers only when a workspace has several splits. It also displays hidden owned buffers, known projects, and unowned native Emacs buffers. Its scratch action creates an explicitly unowned, non-persistent buffer. Its cleanup action uses a single-key confirmation before killing all scratch and unowned buffers. Selecting a buffer enters its owner workspace. Navigator `x` can stop a process buffer and remove its restart record, or close and remove a workspace so no stopped gray entry remains. New splits open independent Dired buffers; `Space =` splits right and `Space -` splits below at the current directory. `Space h/j/k/l` moves left/down/up/right. `Space x` closes the selected split when several exist. In a single split it kills the current ordinary buffer and opens the current workspace root in Dired; a running process is hidden instead of stopped. `M-x compile` starts with a blank command.

Evil normal state uses `Space` as the leader, including Dired and Compile buffers. Ghostel terminals and workspace-owned AIPanel processes use `Ctrl-Escape` to return to normal state; `evil-ghostel` routes plain `Escape` to the terminal application. Ghostel uses libghostty's VT engine and native PTY path, advertises `xterm-256color` for local and remote compatibility, forwards terminal mouse tracking, excludes terminal buffers from editor line highlighting, and uses the Black & Pink 16-color palette formerly configured in Ghostty. Its prebuilt native module downloads without prompting into `$XDG_DATA_HOME/myconfig-emacs/ghostel-module/`. `Space t` opens a terminal, `Space a` toggles the workspace's AIPanel pane, `Space w` opens the navigator, and `Space g g` opens Magit. AIPanel uses a standard Emacs side window dedicated to its process. The standalone `aipan.el` package owns discovery, local process startup, context delivery, and panel lifecycle; `aipanel-atelier.el` optionally connects its lifecycle hooks and strategy functions to Atelier workspace jobs, remote context, and restoration. Either package can load without the other, and the main configuration loads the adapter only when AIPanel is present. When no process is running, AIPanel lists configured agents found through Emacs's normal executable path and in the default WSL distribution, or starts it directly when only one is available. The Emacs startup adds the conventional `~/.local/bin` directory to its process environment once. OpenCode, Claude Code, Codex, and `fx` are configured by default; `aipanel-agents` controls each executable's flags, compact-mode arguments, project argument, and readiness delay independently. Tree-sitter Auto prompts to install missing parsers and selects compatible `*-ts-mode` modes. `M-x mason-manager` manually installs Emacs-owned language servers; Eglot starts automatically only for commands installed under the Emacs Mason directory.
...
The CachyOS profile does not install or configure a separate terminal emulator. It removes the CachyOS Fish stack and legacy terminal packages, then uses Ghostel inside normal-process graphical Emacs for workbench terminals. The same Emacs configuration supports native Windows through Ghostel's prebuilt Windows module and ConPTY backend. The Emacs module disables any legacy user daemon, removes the old client and custom desktop override, and relies on the distribution `emacs.desktop`. Closing the last Emacs frame therefore stops the complete workbench process.
