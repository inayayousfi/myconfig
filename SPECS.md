...
The user package phase installs the latest Playwright MCP package through Bun and downloads only its matching Chromium Headless Shell. The Arch package list includes the browser's required shared libraries. The installer replaces only the `mcp.playwright` entry in `~/.config/opencode/opencode.json` while preserving all other valid JSON settings. Invalid existing JSON is left unchanged with a warning. A failed Playwright MCP or Chromium installation does not stop provisioning; the generated OpenCode entry remains disabled and `~/environment.md` records the failure.
