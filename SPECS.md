...
An existing rEFInd installation receives a matching minimal theme with mouse support, maximum firmware resolution, custom Linux and Windows icons, pink selection rings, and only shutdown, reboot, and firmware tools. The installer keeps a one-time `refind.conf.pre-blacknpink` backup and owns a marked configuration block so reruns preserve unrelated boot entries and settings.

The profile installs a libinput Lua plugin at `/etc/libinput/plugins/90-myconfig-pointer-sensitivity.lua`. KWin loads it at the next login and multiplies relative X/Y pointer motion by 2. It applies to mice, trackballs, and other relative pointers, while absolute touchpad and touchscreen events are unchanged. KDE's pointer and touchpad acceleration remain at their maximum flat-profile setting.
