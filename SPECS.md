...
The workbench runtime hierarchy is workspace, then entry roots and their recursive split trees; persisted version 10 snapshots store flat entry records connected by stable `:id`, `:parent-id`, and split `:child-ids`, with workspace root IDs. Each workspace owns content records with stable IDs; basic entries store only an ordered `:content-ids` list, active first. Loading reconstructs the runtime trees from those references. A top-level split entry is the root of a disposition; it is not wrapped in a separate disposition entry. Root-ness is structural: selecting a nested entry resolves the top-level entry under its workspace before materializing that disposition. Split entries contain split entries or content entries; content entries refer to buffers and may carry registered content `type` metadata such as file, Dired, terminal, or AIPanel. The workspace tree owns entry references, not unique buffer objects: multiple content entries may refer to one live buffer, and the buffer is closed (including its process) only after its final Atelier entry reference is removed. Closing a view that shares its content with another displayed split removes that view and preserves the buffer/process. The entry tree is authoritative; live Emacs buffers carry no Atelier ownership metadata. A split entry's `kind` describes its structural role and owns child entries; a content entry's storage `kind` controls persistence and restoration. An entry's content stack holds the active buffer first and inactive contents behind it; the entry's ID and view placement stay fixed when the active content changes. Its registered `type` classifies that active content, not the view. The navigator renders one row per entry with its contents horizontally; `j`/`k` move between rows, `h`/`l` move across contents within a row; after a short idle pause (0.45 seconds by default) the chosen content rotates to the top of the stack in the same view. Enter or Quit commits a pending choice immediately; Enter opens that view. Navigator `x` closes the selected content; `X` removes the whole entry and all its contents. Registered types control default lookup and derive service-buffer names as `*type:workspace*`. Registered types include file, Dired, terminal, and AIPanel. Automatic buffer discovery does not add another entry when that type already exists. Explicit actions may add another typed entry, which receives Emacs's normal `<2>` suffix: `Space f` from Dired and `Space t` from a terminal create another entry, while those commands from another buffer reopen the newest live entry of that type. Terminals and AIPanel sessions appear as normal workspace entries even though their restart recipes are tracked internally. Each AIPanel is attached to one basic leaf entry by its permanent entry ID, and one source entry may own one panel. Different source entries may run independent panels. Moving a source entry also moves its panel entry; removing the source entry or killing its live buffer stops and removes the panel. Durable workspace-owned file, Dired, and scratch entries are restored after a restart, while transient entries exist only for the current process. Hidden entries remain assigned to their workspace, and `Space b` excludes buffers owned by other workspaces. The full-frame navigator renders each visible layout as its recursive side-by-side and stacked entry tree, preserves geometric child order, and keeps each leaf's real split number and target. Hidden top-level entries follow the visible tree in permanent-ID order. The navigator also displays known projects and unowned native Emacs buffers. Its cleanup action uses a single-key confirmation before killing all scratch and detached buffers. Selecting a buffer enters its owner workspace. Navigator `d` detaches the selected entry; when that entry was the only visible split, Atelier reuses the most recently used surviving workspace entry, restores a saved one if needed, and shows the empty-workspace message when none remains instead of creating another Dired entry. Navigator `x` and `Space x` remove the selected content entry and its restart record. A process stops only when no remaining workspace entry refers to its buffer; when another displayed split shows that same buffer, only the selected window is removed. Closing the only visible reference uses the normal replacement order. New splits explicitly open additional typed Dired entries; `Space =` splits right and `Space -` splits below at the current directory. `Space h/j/k/l` moves left/down/up/right. `M-x compile` starts with a blank command.

Evil normal state uses `Space` as the leader, including Dired and Compile buffers. Ghostel terminals and Atelier-owned AIPanel processes use `M-x` to return to normal state; `evil-ghostel` routes plain `Escape` to the terminal application. Ghostel uses libghostty's VT engine and native PTY path, advertises `xterm-256color` for local and remote compatibility, forwards terminal mouse tracking, excludes terminal buffers from editor line highlighting, and uses the Black & Pink 16-color palette formerly configured in Ghostty. Its prebuilt native module downloads without prompting into `$XDG_DATA_HOME/myconfig-emacs/ghostel-module/`. `Space t` opens a terminal, `Space a` toggles the panel attached to the current source, `Space w` opens the navigator, and `Space g g` opens Magit. AIPanel uses a standard Emacs side window dedicated to its process. Without an integration, the standalone `aipan.el` package attaches a panel to the current buffer and owns matching-environment discovery, process startup, context delivery, source cleanup, and panel lifecycle. `aipanel-atelier.el` optionally replaces the buffer attachment with the current basic Atelier entry and connects panel lifecycle to Atelier jobs, entry movement, remote execution, and restoration. Either package can load without the other, and the main configuration loads the adapter only when AIPanel is present. When no process is running, AIPanel lists only configured agents found in the attached source's execution environment, or starts one directly when only one is available. The agent starts with the attached buffer or entry directory as its working directory. File context retains the current line and column and expresses the file path relative to that exact working directory. Local, WSL, POSIX SSH, and Windows SSH sources launch the agent in the matching environment; AIPanel does not silently combine a source from one environment with an agent from another. The Emacs startup adds the conventional `~/.local/bin` directory to its process environment once. OpenCode, Claude Code, Codex, and `fx` are configured by default; `aipanel-agents` controls each executable's flags, compact-mode arguments, project argument, and readiness delay independently. Tree-sitter Auto prompts to install missing parsers and selects compatible `*-ts-mode` modes. `M-x mason-manager` manually installs Emacs-owned language servers; Eglot starts automatically only for commands installed under the Emacs Mason directory.
...
Local workspaces use normal filesystem paths. Native Windows workspaces use bounded SSHFS mounts and OpenSSH-launched PowerShell rather than TRAMP against a `cmd.exe` login. The workspace creator also supports WSL distributions through TRAMP `/wsl:` paths and `wsl.exe` terminals. AIPanel discovers and launches the selected agent in the attached entry's local, WSL, POSIX SSH, or Windows SSH environment and gives it the entry directory as its working directory. Its file context uses the path relative to that directory plus the current line and column. Context waits for a stable agent screen, and a newer request cancels an older pending send.

Cross-platform operations live in the independent, single-file `univers.el` package under the `universel-` namespace. An explicit platform overrides detection; otherwise operations use their file or connection context. Local configuration paths and local process-launch machinery use the Emacs host. The optional `universel-atelier.el` and `universel-aipanel.el` adapters connect the package to Atelier and AIPanel through replaceable functions. The configuration loads these adapters explicitly and preserves the existing workspace records and feature availability.
