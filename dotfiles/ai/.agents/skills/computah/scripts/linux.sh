...
capture() {
    ...
    [[ -n $backend ]] || {
        printf 'computah: no screenshot backend for %s/%s\n' "$session" "$desktop" >&2
        exit 2
    }
    ...
    [[ -s $output ]] || {
        printf 'computah: screenshot is empty\n' >&2
        exit 2
    }
    printf '%s\n' "$output"
}
...
crop() {
    ...
    has magick || {
        printf 'computah: crop requires ImageMagick\n' >&2
        exit 2
    }
...
require_keyboard() {
    ...
    [[ -n $backend ]] || {
        printf 'computah: no keyboard backend for %s/%s\n' "$session" "$desktop" >&2
        exit 2
    }
    printf '%s' "$backend"
}
...
require_pointer() {
    ...
    [[ -n $backend ]] || {
        printf 'computah: no pointer backend for %s/%s\n' "$session" "$desktop" >&2
        exit 2
    }
    printf '%s' "$backend"
}
...
command=${1:-}
[[ -n $command ]] || {
    printf 'computah: missing command\n' >&2
    exit 2
}
shift
...
case $command in
    ...
    type)
        [[ $# -eq 1 ]] || {
            printf 'computah: type requires one quoted TEXT argument\n' >&2
            exit 2
        }
    ...
    key)
        [[ $# -eq 1 ]] || {
            printf 'computah: key requires CHORD\n' >&2
            exit 2
        }
        backend=$(require_keyboard)
    ...
    move)
        [[ $# -eq 2 ]] || {
            printf 'computah: move requires X Y\n' >&2
            exit 2
        }
    ...
    click)
        [[ $# -ge 2 && $# -le 3 ]] || {
            printf 'computah: click requires X Y [BUTTON]\n' >&2
            exit 2
        }
        backend=$(require_pointer)
        button=${3:-left}
        if [[ $backend == ydotool ]]; then
            case $button in left) button=0xC0 ;; right) button=0xC1 ;; middle) button=0xC2 ;; *)
                printf 'computah: unknown button\n' >&2
                exit 2
                ;;
            esac
            ...
            case $button in left) button=1 ;; middle) button=2 ;; right) button=3 ;; *)
                printf 'computah: unknown button\n' >&2
                exit 2
                ;;
            esac
    ...
    drag)
        [[ $# -ge 4 && $# -le 5 ]] || {
            printf 'computah: drag requires X1 Y1 X2 Y2 [BUTTON]\n' >&2
            exit 2
        }
        backend=$(require_pointer)
        button=${5:-left}
    ...
    scroll)
        [[ $# -eq 1 && $1 =~ ^-?[0-9]+$ ]] || {
            printf 'computah: scroll requires integer NOTCHES\n' >&2
            exit 2
        }
        backend=$(scroll_backend)
        [[ -n $backend ]] || {
            printf 'computah: no scroll backend for %s/%s\n' "$session" "$desktop" >&2
            exit 2
        }
        notches=$1
        button=4
